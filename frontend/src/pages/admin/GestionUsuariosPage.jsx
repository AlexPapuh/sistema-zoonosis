...
import {
  ...
  Plus, Edit, Trash2, Filter, ChevronLeft, ChevronRight, X, Save, Lock, MapPin, CheckCircle
} from 'lucide-react';
...
const StatCard = ({ label, count, icon, color }) => (
    <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 flex items-center justify-between">
        ...
        <div className={`p-3 rounded-lg text-white ${color} shadow-md`}>{icon}</div>
    </div>
);
...
const GestionUsuariosPage = () => {
  ...
  const handleCreateUser = async (e) => {
      ...
      try {
          ...
          setNewUser({ nombre: '', email: '', password: '', rol: 'Propietario', telefono: '', ci: '', direccion: '', distrito: '', latitud: '', longitud: '' });
  ...
  const handleReactivate = (id) => {
    Swal.fire({
      ...
      text: "El usuario recuperará su acceso al sistema de forma inmediata.",
      icon: 'question',
      showCancelButton: true,
      ...
      if (result.isConfirmed) {
        try {
          ...
          console.error("=== DETECTANDO ERROR DE REACTIVACIÓN ===");
          console.error(error);
    ...
    <div className="container mx-auto pb-10 px-4">
      ...
      <div className="space-y-4 mb-6">
        <div className="flex justify-between items-center">
            <h3 className="text-gray-500 font-medium">Usuarios Registrados ({filteredUsers.length})</h3>
            <span className="text-xs text-gray-400">Página {currentPage} de {totalPages || 1}</span>
        </div>
        {currentUsers.map(user => (
            ...
            >
                ...
                <div className="flex items-center gap-3 mt-4 md:mt-0 w-full md:w-auto justify-end">
                    <button onClick={() => openEditModal(user)} className="p-2 text-blue-600 hover:bg-blue-50 rounded-lg border border-transparent hover:border-blue-100 transition-all" title="Editar">
                        <Edit size={18}/>
                    </button>
                    
                    {user.estado !== 'Inactivo' ? (
                        <button onClick={() => handleDelete(user.id)} className="p-2 text-red-600 hover:bg-red-50 rounded-lg border border-transparent hover:border-red-100 transition-all" title="Desactivar">
                            <Trash2 size={18}/>
                        </button>
                    ) : (
                        <button onClick={() => handleReactivate(user.id)} className="p-2 text-green-600 hover:bg-green-50 rounded-lg border border-transparent hover:border-green-100 transition-all" title="Reactivar Usuario">
                            <CheckCircle size={18}/>
                        </button>
                    )}
                </div>
           ...
           <div className="bg-white rounded-xl shadow-xl p-6 w-full max-w-lg relative animate-in fade-in zoom-in duration-200">
             ...
             <form onSubmit={handleUpdateUser} className="space-y-4">
                <div>
                    ...
                    <input type="text" name="nombre" className="w-full border rounded-lg p-2.5 focus:ring-2 focus:ring-blue-500 outline-none" value={editingUser.nombre} onChange={handleEditChange} required/>
                </div>
                <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Correo Electrónico</label>
                    <input type="email" name="email" className="w-full border rounded-lg p-2.5 focus:ring-2 focus:ring-blue-500 outline-none" value={editingUser.email} onChange={handleEditChange} required/>
                </div>
                
                <div className="grid grid-cols-2 gap-4">
                    <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">Rol</label>
                        <select name="rol" className="w-full border rounded-lg p-2.5 bg-white" value={editingUser.rol} onChange={handleEditChange}>
                    ...
                    <div>
                        ...
                        <input type="text" name="telefono" className="w-full border rounded-lg p-2.5 focus:ring-2 focus:ring-blue-500 outline-none" value={editingUser.telefono || ''} onChange={handleEditChange}/>
                    </div>
                </div>
                ...
                <div className="flex justify-end pt-4 space-x-3">
                    ...
                    <button type="submit" className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 flex items-center shadow-md">
           ...
           <div className="bg-white rounded-xl shadow-xl p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto relative animate-in fade-in zoom-in duration-200">
             ...
             <form onSubmit={handleCreateUser} className="space-y-3">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">Nombre Completo</label>
                        <input type="text" name="nombre" className="w-full border rounded-lg p-2.5 focus:ring-2 focus:ring-green-500 outline-none" value={newUser.nombre} onChange={handleCreateChange} required/>
                    </div>
                    <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">Correo Electrónico</label>
                        <input type="email" name="email" className="w-full border rounded-lg p-2.5 focus:ring-2 focus:ring-green-500 outline-none" value={newUser.email} onChange={handleCreateChange} required/>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <div className="relative">
                            ...
                            <input type="password" name="password" className="w-full border rounded-lg pl-10 pr-4 py-2.5 focus:ring-2 focus:ring-green-500 outline-none" value={newUser.password} onChange={handleCreateChange} required/>
                        </div>
                    </div>
                    <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">Rol</label>
                        <select name="rol" className="w-full border rounded-lg p-2.5 bg-white focus:ring-2 focus:ring-green-500" value={newUser.rol} onChange={handleCreateChange}>
                            <option value="Propietario">Propietario</option>
                            <option value="Veterinario">Veterinario</option>
                            <option value="Admin">Administrador</option>
                        </select>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">Teléfono</label>
                        <input type="text" name="telefono" className="w-full border rounded-lg p-2.5 focus:ring-2 focus:ring-green-500 outline-none" value={newUser.telefono} onChange={handleCreateChange}/>
                    </div>
                    <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">Cédula Identidad</label>
                        <input type="text" name="ci" className="w-full border rounded-lg p-2.5 focus:ring-2 focus:ring-green-500 outline-none" value={newUser.ci} onChange={handleCreateChange}/>
                    </div>
                </div>

                {newUser.rol === 'Propietario' && (
                    <div className="border-t pt-3 mt-2">
                        <h3 className="text-md font-bold text-gray-700 mb-2 flex items-center">
                            <MapPin size={18} className="mr-2 text-blue-500"/> Ubicación y Distrito
                        </h3>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-3">
                            <div>
                                ...
                                <select 
                                    name="distrito" 
                                    className="w-full border border-gray-300 rounded-lg p-2.5 bg-white cursor-pointer focus:ring-2 focus:ring-green-500" 
                                    value={newUser.distrito} 
                            ...
                            <div>
                                ...
                                <input type="text" name="direccion" className="w-full border rounded-lg p-2.5 focus:ring-2 focus:ring-green-500 outline-none" value={newUser.direccion} onChange={handleCreateChange}/>
                            </div>
                        </div>
                        ...
                        <div className="h-40 w-full rounded-lg overflow-hidden border border-gray-300 relative z-0 shadow-sm">
                            ...
                            <MapContainer center={[-19.5894, -65.7541]} zoom={14} style={{ height: '100%', width: '100%' }}>
                                ...
                                <LocationPicker onLocationSelected={handleMapClick} position={newUser.latitud ? [newUser.latitud, newUser.longitud] : null} />
                            </MapContainer>
                        </div>
                        {newUser.latitud && (
                            <p className="text-xs text-green-600 mt-1 font-semibold text-center">
                                Coordenadas: {newUser.latitud.toFixed(5)}, {newUser.longitud.toFixed(5)}
                            </p>
                        )}
                    </div>
                )}
                ...
                <div className="flex justify-end pt-3 space-x-3">
                    <button type="button" onClick={() => setShowCreateModal(false)} className="px-4 py-2 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50">Cancelar</button>
                    <button type="submit" className="px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 flex items-center shadow-md">
